...
class TestMortal(unittest.TestCase):
  ...
  def test_grown_messages(self):
    self.e.add_handler("grown",
                       lambda e,p: e.add_attribute("test", p["new_etape"]))
    for i in range(6):
      if i in self.s.age_etapes:
  ...
  def test_died_message(self):
    self.s.age_etapes = {1: "dead"}
    self.e.add_handler("died",
                       lambda e,p: e.add_attribute("test", p["reason"]))
    self.e.update()
    self.assertEqual(self.e.attribute("test"), "senescence")
